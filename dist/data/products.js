...
export function getProductItemsOfUser(userId) {
    return createPromiseWithDBQuery('SELECT pi.id AS productInterestId, p.id AS productId, pi.userId, p.name AS productName FROM productinterests AS pi INNER JOIN products AS p ON pi.productId = p.id WHERE pi.userId = ?', userId, (resolve, result) => resolve(result));
}
